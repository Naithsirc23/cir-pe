...
export default function Home() {
  ...
  const sync = trpc.projects.sync.useMutation({ onSuccess: () => projectsQuery.refetch() });
  ...
  return <div className="page-enter">
    <header className="page-heading overview-heading">
      ...
      <Button onClick={() => sync.mutate()} disabled={sync.isPending} className="primary-action"><CloudDownload className={`h-4 w-4 ${sync.isPending ? "animate-spin" : ""}`} />{sync.isPending ? "Sincronizando..." : "Sincronizar GitHub"}</Button>
    </header>
    {projectsQuery.isLoading ? <DashboardLoading /> : projectsQuery.isError ? <QueryErrorNotice onRetry={() => projectsQuery.refetch()} /> : !projects.length ? <EmptySync onSync={() => sync.mutate()} pending={sync.isPending} /> : <>
